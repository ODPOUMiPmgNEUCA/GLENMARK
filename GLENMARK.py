...
if df_file:
    try:
        ...
        # Filtrujemy tylko dane o rodzaju promocji 'IPRA'
        df = df[df['Rodzaj promocji'] == 'IPRA']

        # Grupujemy dane i sumujemy wartości
        df = df.groupby(['Kod pocztowy', 'Indeks', 'Nazwa towaru']).agg({
            'Ilość sprzedana': 'sum',
            'Wartość sprzedaży': 'sum'
        }).reset_index()

        # Ustal, czy kod pocztowy znajduje się na liście
        df['Czy w liście'] = df['Kod pocztowy'].isin(lista['Kod pocztowy'])

        # Przypisz dane z listy aptek do df1
        df1 = df[df['Czy w liście'] == True]
        df1 = df1.merge(lista[['Kod pocztowy', 'SAP', 'Nazwa apteki', 'Miejscowość', 'Ulica', 'Nr domu']].drop_duplicates(), on='Kod pocztowy', how='left')

        # Przypisz do df2 te, które nie są na liście
        df2 = df[df['Czy w liście'] == False]

        # Funkcja do dopasowania kodów pocztowych
        def dopasuj_inny_kod_pocztowy(df, kolumna_kodu, kody):
            # Tworzymy słownik, aby przechowywać liczbę użyć każdego kodu
            liczba_uzyc = {kod: 0 for kod in kody}
        ...
        # Użycie funkcji do dopasowania kodów pocztowych
        df_dopasowany = dopasuj_inny_kod_pocztowy(df2, 'Kod pocztowy', lista['Kod pocztowy'].unique().tolist())

        # Łączenie danych z listy aptek do dopasowanych kodów
        df_dopasowany = df_dopasowany.merge(lista[['Kod pocztowy', 'SAP', 'Nazwa apteki', 'Miejscowość', 'Ulica', 'Nr domu']].drop_duplicates(), left_on='dopasowany_kod', right_on='Kod pocztowy', how='left', suffixes=('', '_dopasowany'))

        # Wykonaj grupowanie po dopasowanych kodach, aby upewnić się, że każda apteka jest przypisana tylko raz
        df_dopasowany = df_dopasowany.groupby(['dopasowany_kod', 'SAP', 'Nazwa apteki', 'Miejscowość', 'Ulica', 'Nr domu']).agg({
            'Ilość sprzedana': 'sum',
            'Wartość sprzedaży': 'sum'
        }).reset_index()

        df_dopasowany.rename(columns={'dopasowany_kod': 'Kod pocztowy'}, inplace=True)

        # Uporządkowanie kolumn
        df1.drop(columns=['Czy w liście'], inplace=True)
        df1 = df1[['SAP', 'Nazwa apteki', 'Miejscowość', 'Ulica', 'Nr domu', 'Kod pocztowy', 'Indeks', 'Nazwa towaru', 'Ilość sprzedana', 'Wartość sprzedaży']]
        
        # Łączymy dane df1 i df_dopasowany
        wynik = pd.concat([df1, df_dopasowany], ignore_index=True)

        # Przypisanie roku i miesiąca wystawienia
        wynik['Rok wystawienia'] = datetime.datetime.now().year
        wynik['Miesiąc wystawienia'] = datetime.datetime.now().month

        # Określenie nowego porządku kolumn
        new_order = ['SAP', 'Nazwa apteki', 'Miejscowość', 'Ulica', 'Nr domu', 'Kod pocztowy', 'Indeks', 'Nazwa towaru', 'Ilość sprzedana', 'Wartość sprzedaży', 'Rok wystawienia', 'Miesiąc wystawienia']
        wynik = wynik[new_order]

        # Zapisz raport do pliku Excel
        dzisiejsza_data = datetime.datetime.now().strftime("%d.%m.%Y")
        excel_file = io.BytesIO()
        with pd.ExcelWriter(excel_file, engine='xlsxwriter') as writer:
            wynik.to_excel(writer, index=False, sheet_name='Sheet1')
        ...
        st.download_button(
            label='PLIK RAPORTU vol2',
            data=excel_file,
            file_name=f"RAPORT GLENMARK_{dzisiejsza_data}.xlsx",
            mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
        )
